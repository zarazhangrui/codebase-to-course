...
> - **A local checkout of a GitHub repo** - e.g., "turn ./repo-clone into a course"
...
If the user provides a GitHub link, do **not** auto-clone it. Treat external repositories as untrusted input.

Safe workflow:
1. Ask the user to confirm the repository is trusted and available locally.
2. Analyze only local directories provided by the user.
3. If the user only provides a URL, ask them to clone it themselves and share the local path.
4. Never execute code from the analyzed repository.
...
**Critical: Preserve logic fidelity while protecting secrets.** Keep snippets faithful to the real implementation so learners can map explanations back to source files, but never expose sensitive data. Before including any snippet, scan and redact credentials/secrets (API keys, tokens, passwords, private URLs, connection strings, `.env` values, auth headers, signed cookies, private cert material, etc.). If a secret appears inline, replace only the secret value with `[REDACTED]` and label it as redacted.

Do not simplify away important control flow. Instead of heavily editing long functions, prefer naturally short snippets (5-10 lines) that illustrate the concept clearly.
...
### Code Modifications
Over-editing snippets can hide how the system really works. Keep snippets faithful so learners can match them to source files, but always redact any sensitive values. Instead of rewriting logic to be shorter, *choose* naturally short snippets (5-10 lines) from the codebase that illustrate the point.
...
### Secret Leakage in Snippets
Copying code without a safety pass can leak real credentials. Always run a snippet safety check before rendering course content:
- Redact obvious secrets and high-entropy credential-like strings.
- Remove or mask any personal data not needed for teaching.
- Do not include `.env` files, secret managers exports, private keys, or credential dumps in course material.
- If a concept requires showing auth configuration, use placeholders (e.g., `API_KEY=[REDACTED]`).

Trust is built by accurate explanations, not by exposing raw secrets.
